...
function validerafectermission() {
    $("#messageerreur").empty();
    var messageerreur = [];
    var element=0;
   if($(".missionid").length>1){
    for(var i = 0; i <document.affectermission.missionid.length; i++) {
   if(document.affectermission.missionid[i].checked == true) {
       element++;
   }
   }
   if(element==0){
   messageerreur.push('Veuillez sélectionner une mission');
   }
   }
   else if($(".missionid").length==1 && !$('#missionid').is(':checked')){
   messageerreur.push('Veuillez s&eacute;lectionner une mission');
   }
   else if($(".missionid").length==0){
   messageerreur.push('la liste ne contient aucun &eacute;l&eacute;ment');   
   }
