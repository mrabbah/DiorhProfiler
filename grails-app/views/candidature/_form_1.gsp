...
<div class="tabbable tabbable-bordered">
  <ul class="nav nav-tabs">
    ...
    <li><a href="#tab_br2" data-toggle="tab">Avancement de la candidature1</a></li>
  </ul>
        ...
        <div class='row'>
          <div class="panel panel-danger">
            <div class="panel-body">
              ...
              <div class="col-sm-3"><g:select class="form-control" id="statut" name="statut.id" from="${com.rabbahsoft.diorhprofiler.StatutCandidature.list()}" optionKey="id" value="${candidatureInstance?.statut?.id}" class="form-control" noSelection="['null': 'Sélectionner...']"/></div>
<!--              <div class="col-sm-3 showvalue">${candidatureInstance?.statut}</div>-->
<!--              <div class="col-sm-3"><select name="statut" id="statut" class="form-control">
            <option value="En cours" <g:if test="${candidatureInstance?.statut?.libelle=="En cours"}">selected</g:if>>En cours</option>
            <option value="Sélectionnée" <g:if test="${candidatureInstance?.statut?.libelle=="Sélectionnée"}">selected</g:if>>Sélectionnée</option>
            <option value="En attente" <g:if test="${candidatureInstance?.statut?.libelle=="En attente"}">selected</g:if>>En attente</option>
            <option value="Acceptée" <g:if test="${candidatureInstance?.statut?.libelle=="Acceptée"}">selected</g:if>>Acceptée</option>
            <option value="Validée" <g:if test="${candidatureInstance?.statut?.libelle=="Validée"}">selected</g:if>>Validée</option>
            <option value="Recrutée" <g:if test="${candidatureInstance?.statut?.libelle=="Recrutée"}">selected</g:if>>Recrutée</option>
            <option value="Non retenue" <g:if test="${candidatureInstance?.statut?.libelle=="Non retenue"}">selected</g:if>>Non retenue</option>
